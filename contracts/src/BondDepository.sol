...
/// @notice Two markets: 0 = USDG reserve bonds, 1 = MOASS/USDG v2 LP bonds.
...
contract BondDepository is IBondDepository, Wired {
    ...
    /// @notice MOASS payout already committed per epoch index.
    mapping(uint256 => uint256) public payoutInEpoch;

    constructor(address moass_, address usdg_, address pair_, address oracle_, address treasury_) {
    ...
    function bondPrice(uint256 marketId) public view returns (uint256) {
        ...
        uint256 discounted =
            oracle.twapMoassUsdg() * (Constants.BPS - Constants.BOND_DISCOUNT_BPS) / Constants.BPS;
        ...
        returns (uint256 noteId, uint256 payout)
    {
        ...
        payout = FixedPointMath.mulDiv(valueWad, Constants.MOASS_UNIT, price);
        _checkEpochCap(payout);

        treasury.mintMoass(address(this), payout);
    ...
    function redeem(address to) external returns (uint256 paid) {
        ...
        if (paid != 0 && !moass.transfer(to, paid)) revert TransferFailed();
    }
    ...
    ///      2·√(xWad·yWad), MOASS leg at 1 USDG intrinsic.
    ...
    function _checkEpochCap(uint256 payout) internal {
        ...
        uint256 cap = moass.totalSupply() * Constants.BOND_EPOCH_CAP_BPS / Constants.BPS;
