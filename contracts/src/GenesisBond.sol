...
/// @notice Fixed-price offering (3 USDG/MOASS); hard cap, wallet cap, sale
...
contract GenesisBond is IGenesisBond, Wired {
    ...
    mapping(address => uint256) public claimedMoass;
    ...
    function purchase(uint256 usdgAmountRaw) external {
        ...
        emit Purchased(
            msg.sender, amountWad, amountWad * Constants.MOASS_UNIT / Constants.GENESIS_PRICE_WAD
        );
    }
    ...
    function finalize() external {
        ...
        // 2. Pair the remaining ~30% with newly minted MOASS at GENESIS_PRICE
        ...
        uint256 moassForPol =
            FixedPointMath.mulDiv(polWad, Constants.MOASS_UNIT, Constants.GENESIS_PRICE_WAD);
        treasury.mintMoass(address(this), moassForPol);
        if (!moass.transfer(address(pairContract), moassForPol)) revert TransferFailed();
        if (!usdg.transfer(address(pairContract), polRaw)) revert TransferFailed();
        pairContract.mint(address(treasury));

        // 3. Mint the bonders' MOASS into the 5-day vesting escrow.
        uint256 moassSold =
            FixedPointMath.mulDiv(raisedWad, Constants.MOASS_UNIT, Constants.GENESIS_PRICE_WAD);
        ...
        moass.enableTax(address(pairContract));
        oracle.checkpoint();
    ...
    function claim() external returns (uint256 moassAmount) {
        ...
        moassAmount = claimableMoassOf(msg.sender);
        if (moassAmount == 0) return 0;
        claimedMoass[msg.sender] += moassAmount;
        if (!moass.transfer(msg.sender, moassAmount)) revert TransferFailed();
        emit Claimed(msg.sender, moassAmount);
    }

    /// @inheritdoc IGenesisBond
    function purchasedMoassOf(address account) public view returns (uint256) {
        return FixedPointMath.mulDiv(
            purchasedRaw[account] * _usdgWadFactor, Constants.MOASS_UNIT, Constants.GENESIS_PRICE_WAD
        );
    }

    /// @inheritdoc IGenesisBond
    function claimableMoassOf(address account) public view returns (uint256) {
        if (!finalized) return 0;
        uint256 total = purchasedMoassOf(account);
        uint256 elapsed = block.timestamp - finalizeTime;
        uint256 vested =
            elapsed >= Constants.GENESIS_VEST ? total : total * elapsed / Constants.GENESIS_VEST;
        return vested - claimedMoass[account];
    }
