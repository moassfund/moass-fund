...
/// @notice Uniswap v2 cumulative-price oracle over the canonical MOASS/USDG
...
contract PairOracle is IPairOracle {
    ...
    /// @notice True when MOASS is token0 of the pair.
    bool public immutable moassIsToken0;
    /// @dev Scale factor from UQ112 raw ratio to WAD USDG per whole MOASS:
    ///      10^(18 + moassDecimals − usdgDecimals).
    uint256 public immutable priceScale;

    Observation[SLOTS] public observations;
    uint64 public lastCheckpointAt;

    constructor(address pair_, address moass_, address usdg_) {
        ...
        moassIsToken0 = IUniswapV2Pair(pair_).token0() == moass_;
    ...
    /// @dev Counterfactual current cumulative price of MOASS in the quote
    ...
    function _currentCumulative() internal view returns (uint256 cum) {
        cum =
            moassIsToken0 ? pairContract.price0CumulativeLast() : pairContract.price1CumulativeLast();
        ...
        if (tsLast != nowTs && r0 != 0 && r1 != 0) {
            ...
            (uint112 quoteReserve, uint112 moassReserve) = moassIsToken0 ? (r1, r0) : (r0, r1);
            unchecked {
                cum += (uint256(quoteReserve) << 112) / moassReserve * delta;
            }
        }
    }
}
