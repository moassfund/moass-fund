...
///      - MOASS amounts: 9 decimals (native).
...
///        MOASS (1e9 base units).
interface ITreasury {
    event MorphoDeposited(uint256 assets);
    event MorphoWithdrawn(uint256 assets);
    event MoassMinted(address indexed minter, address indexed to, uint256 amount);
    ...
    /// @notice rfv() / totalSupply, WAD USDG per whole MOASS. The protocol NAV.
    ...
    /// @notice Mints MOASS to `to`. Authorized minters only (fixed at deploy:
